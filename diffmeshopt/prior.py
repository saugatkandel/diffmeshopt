...
def sample_volume_along_normals(
    mesh: Meshes, volume: torch.Tensor, num_samples: int, sample_distance: float
):
    """
    ...
    line_points = torch.linspace(
        -sample_distance / 2, sample_distance / 2, num_samples, device=verts.device
    )
...
def gaussian_prior_loss(
    ...
    intensity_profiles = sample_volume_along_normals(mesh, volume, num_samples, sample_distance)
